...
var test = function(status) {
    ...
    removeTree(diffsPath);

    if (diff) {
        makeDirectory(diffsPath);
    }
    ...
    for (var i = 0; i < demos.length; i += 1) {
        ...
        if (exists(worldStartPath)) {
            var worldStartRef = JSON.parse(read(worldStartPath));
            var worldStartDiff = compare(worldStartRef, worldStart);

            if (worldStartDiff.length !== 0) {
                if (diff) {
                    write(worldStartDiffPath, JSON.stringify(worldStartDiff, precisionLimiter, 2), 'w');
                }

                if (forceUpdate) {
                    hasCreated = true;
                    write(worldStartPath, JSON.stringify(worldStart, precisionLimiter, 2), 'w');
                } else {
                    hasChanged = true; 
                }
            }
        ...
        if (exists(worldEndPath)) {
            var worldEndRef = JSON.parse(read(worldEndPath));
            var worldEndDiff = compare(worldEndRef, worldEnd);

            if (worldEndDiff.length !== 0) {
                if (diff) {
                    write(worldEndDiffPath, JSON.stringify(worldEndDiff, precisionLimiter, 2), 'w');
                }

                if (forceUpdate) {
                    hasCreated = true;
                    write(worldEndPath, JSON.stringify(worldEnd, precisionLimiter, 2), 'w');
                } else {
                    hasChanged = true;
                }
            }
